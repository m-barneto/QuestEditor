...
export const QuestEditorForm: FC<QuestEditorFormProps> = (props): JSX.Element => {
    ...
    const { localeActiveIndices, setLocaleActiveIndices, questTabIndex, setQuestTabIndex } = useContext(UserLayoutContext)!;
                    ...
                    <TabPanel header="Basics">
                        ...
                        <Accordion style={{ paddingTop: "1rem" }} activeIndex={localeActiveIndices} multiple onTabChange={(accordianProps) => {
                            ...
                            <AccordionTab header="Description">
                                <InputTextarea autoResize value={locales[quest.description]} onChange={(e) => updateLocale(quest.description, e.target.value)} rows={5} style={{ width: "100%" }} />
                            </AccordionTab>
                            <AccordionTab header="Started Message">
                                <InputTextarea autoResize value={locales[quest.startedMessageText]} onChange={(e) => updateLocale(quest.startedMessageText, e.target.value)} rows={5} style={{ width: "100%" }} />
                            </AccordionTab>
                            <AccordionTab header="Fail/Success Message">
                                <FloatLabel style={{ marginTop: "1rem" }}>
                                    <InputTextarea id="successMessageText" autoResize value={locales[quest.successMessageText]} onChange={(e) => updateLocale(quest.successMessageText, e.target.value)} rows={5} style={{ width: "100%" }} />
                                    <label htmlFor="successMessageText">Quest Success Message</label>
                                </FloatLabel>
                                <FloatLabel style={{ marginTop: "1.5rem" }}>
                                    <InputTextarea id="failMessageText" autoResize value={locales[quest.failMessageText]} onChange={(e) => updateLocale(quest.failMessageText, e.target.value)} rows={5} style={{ width: "100%" }} />
                                    <label htmlFor="failMessageText">Quest Fail Message</label>
                                </FloatLabel>
                            </AccordionTab>
                            <AccordionTab header="Unused Messages">
                                <FloatLabel style={{ marginTop: "1rem" }}>
                                    <InputTextarea id="acceptPlayerMessage" autoResize value={quest.acceptPlayerMessage ? locales[quest.acceptPlayerMessage] : ""} onChange={(e) => updateLocale(quest.acceptPlayerMessage, e.target.value)} rows={5} style={{ width: "100%" }} />
                                    <label htmlFor="acceptPlayerMessage">Accept Player Message</label>
                                </FloatLabel>
                                <FloatLabel style={{ marginTop: "1.5rem" }}>
                                    <InputTextarea id="failMessageText" autoResize value={locales[quest.failMessageText]} onChange={(e) => updateLocale(quest.failMessageText, e.target.value)} rows={5} style={{ width: "100%" }} />
                                    <label htmlFor="failMessageText">Quest Fail Message</label>
                                ...
                                <FloatLabel style={{ marginTop: "1.5rem" }}>
                                    <InputTextarea id="changeQuestMessageText" autoResize value={locales[quest.changeQuestMessageText]} onChange={(e) => updateLocale(quest.changeQuestMessageText, e.target.value)} rows={5} style={{ width: "100%" }} />
                                    <label htmlFor="changeQuestMessageText">Change Quest Message</label>
                                </FloatLabel>
                            </AccordionTab>
                        </Accordion>
                    </TabPanel>
...
{/*                        <Divider/>
                        ...
                        <Accordion style={{ paddingTop: "1rem" }} multiple>
                            ...
                            <AccordionTab header="AvailableForFinish">
                                
                            </AccordionTab>
                            <AccordionTab header="FailRestartable">
                                
                            </AccordionTab>
                            <AccordionTab header="Expired">
                    ...
                    <TabPanel header="Misc">
                        <Panel header="Optional (May not function as expected!)" className="flex-wrap justify-content-center gap-3">
                            <div className="grid">
                                <div className="col-4 mb-4">
                                    <Checkbox inputId="canshownoficationsingame"
                                        checked={quest.canShowNotificationsInGame}
                                        onChange={(e) => setQuest({ ...quest, canShowNotificationsInGame: e.checked! })}
                                    />
                                    <label htmlFor="canshownoficationsingame" className="ml-2">Show Notifications In Game</label>
                                </div>
                                <div className="col-4 mb-4">
                                    <Checkbox inputId="iskey"
                                        checked={quest.isKey}
                                        onChange={(e) => setQuest({ ...quest, isKey: e.checked! })}
                                    />
                                    <label htmlFor="iskey" className="ml-2">Is Key Quest</label>
                                </div>
                                <div className="col-4 mb-4">
                                    <Checkbox inputId="restartable"
                                        checked={quest.restartable}
                                        onChange={(e) => setQuest({ ...quest, restartable: e.checked! })}
                                    />
                                    <label htmlFor="restartable" className="ml-2">Restartable</label>
                                </div>
                                <div className="col-4">
                                    <Checkbox inputId="instantComplete"
                                        checked={quest.instantComplete}
                                        onChange={(e) => setQuest({ ...quest, instantComplete: e.checked! })}
                                    />
                                    <label htmlFor="instantComplete" className="ml-2">Instant Complete</label>
                                </div>
                                <div className="col-4">
                                    <Checkbox inputId="secretQuest"
                                        checked={quest.secretQuest}
                                        onChange={(e) => setQuest({ ...quest, secretQuest: e.checked! })}
                                    />
                                    <label htmlFor="secretQuest" className="ml-2">Secret Quest</label>
                                </div>
                                <div className="col-4" />
                            </div>
                            <FloatLabel style={{ marginTop: "2rem" }}>
                                <InputTextarea id="note" autoResize value={locales[quest.note]} onChange={(e) => updateLocale(quest.note, e.target.value)} rows={5} style={{ width: "100%" }} />
